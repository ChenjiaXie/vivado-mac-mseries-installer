...
## ✨ 核心特性与优化
* **全面支持 M 系列芯片**：经过测试并优化，完美兼容 M1, M2, M3, **M4**, 以及即将发布的 **M5** 芯片，充分发挥单核性能优势。
* **原生支持离线安装包 (.tar)**：智能识别几十个 G 的 `.tar` 离线完整安装包（Single File Download）。自动跳过繁琐的 AMD 服务器联网鉴权和命令行登录流程，节省大量时间。
* **真正的“无人值守”静默安装**：移除了原版中繁杂的手动交互（如拖拽文件、手动同意 EULA、输入分辨率等），一键执行脚本，全自动后台完成配置与安装。
* **适配最新 Vivado 组件**：移除了 2025.x 等新版本安装包中不再支持的过时组件（如旧版 Vitis Model Composer 配置），彻底消灭配置文件解析报错。
